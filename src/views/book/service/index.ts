...
export default class BookService {
  ...
  static async requestBookList(
    ...
    await BookService.bookStore.getBooksByThirdCtgyId(
      thirdCtgyId,
      sortField,
      ascOrDesc,
    )
    if (
      ShopCartService.shopStore.getShopCartList &&
      ShopCartService.shopStore.getShopCartList.length === 0
    ) {
      await ShopCartService.requestShopCartList()
    }
    BookService.uptBookNumWithSCLstNum()
  }
  static async requestFullBookList(
    secondctgyId: number,
    sortField: string = 'originalprice',
    ascOrDesc: string = 'asc',
  ) {
    if (sortField === 'price') sortField = 'originalprice'
    await BookService.bookStore.getBooksBySecCtgyId(
      secondctgyId,
      sortField,
      ascOrDesc,
    )
  ...
  static sortBook(_sortField: string) {
    ...
    if (_sortField === 'price')
      BookService.isReadyAsc.value = !BookService.isReadyAsc.value
    BookService.ascOrDesc.value =
      BookService.ascOrDesc.value === 'desc' ? 'asc' : 'desc'
    const { getCurrentThrdCtgy, getCurrentSecondCtgy } = CtgyService.ctgyStore
    if (getCurrentThrdCtgy.thirdctgyId === 10086) {
      BookService.requestFullBookList(
        getCurrentSecondCtgy.secondctgyId,
        BookService.sortField.value,
        BookService.ascOrDesc.value,
      )
    } else {
      BookService.requestBookList(
        getCurrentThrdCtgy.thirdctgyId,
        BookService.sortField.value,
        BookService.ascOrDesc.value,
      )
    }
